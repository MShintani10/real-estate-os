...
get_default_branch() {
    local repo="$1"
    local response
    response=$(github_api_get "$repo" "/repos/${repo}" true 2>/dev/null) || true
    if [[ -n "$response" ]]; then
        printf '%s' "$response" | jq -r '.default_branch // "main"'
    else
        echo "main"
    fi
}
...
setup_repo() {
    ...
    if [[ -d "$repo_path/.git" ]]; then
        ...
        if [[ -n "${IGNITE_WORKER_ID:-}" ]] && [[ -d "$primary_path/.git" ]]; then
            ...
            local bot_token=""
            bot_token=$(get_cached_bot_token "$repo" 2>/dev/null) || true
            if [[ -n "$bot_token" ]]; then
                log_info "GitHub App Token を使用してclone"
                git -c http.extraHeader="Authorization: token ${bot_token}" clone \
                    --branch "$branch" "https://github.com/${repo}.git" "$repo_path"
            else
                git clone "https://github.com/${repo}.git" "$repo_path" --branch "$branch"
            fi
        fi
        cd "$repo_path"
    fi
